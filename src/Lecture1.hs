...
helper n list = go 0 0 list
    where
        go :: Int -> Int -> [Int] -> (Int, Int)
        go greater lower lst 
            | null lst      = (greater, lower)
            | n < head lst  = go greater (lower+1) (tail lst)
            | n > head lst  = go (greater+1) lower (tail lst)
            | otherwise     = go greater lower (tail lst)  
